class Search < ActiveRecord::Base
	def search_refreshes
		...
		refreshes = refreshes.where(["expiry_date <= ?",expiry_date_min]) if expiry_date_min.present?
		...
		refreshes = refreshes.where(["start_date <= ?",start_datae_min]) if start_datae_min.present?
		return refreshes
	end
end